...
pub enum IpcCommand {
    ...
    SetModel {
        ...
        // Runtime catalog overrides (see src/ipc/contract.ts SetModel).
        #[serde(rename = "contextWindow", skip_serializing_if = "Option::is_none")]
        context_window: Option<u64>,
        #[serde(rename = "maxOutputTokens", skip_serializing_if = "Option::is_none")]
        max_output_tokens: Option<u64>,
        // Fast-mode toggle (V0.3 cost + control). Persisted by the frontend
        // (Settings.defaultFastMode) and pushed here so the Node bridge can
        // persist + restore it on reconnect. The daemon CLI has no
        // setFastMode API yet, so the bridge stops at persistence — see
        // the matching TODO in bridge/src/rpc.ts.
...
mod tests {
    ...
    #[test]
    fn setmodel_round_trips_fast_mode_and_runtime_overrides() {
        // The frontend pushes fastMode + runtime catalog overrides on the
        // setModel command (V0.3 cost + control). The Rust mirror must
        // round-trip them so the Node bridge can persist + apply them.
        let json = r#"{"method":"setModel","params":{"provider":"ollama-cloud","model":"deepseek-v4-flash:0731-cloud","thinking":"high","fastMode":true,"contextWindow":1000000,"maxOutputTokens":65536}}"#;
        let cmd: IpcCommand = serde_json::from_str(json).unwrap();
        // Serialize once before destructuring so we don't borrow after move.
        let reser = serde_json::to_string(&cmd).unwrap();
        match cmd {
            IpcCommand::SetModel {
                provider,
                model,
                ...
                assert_eq!(model, "deepseek-v4-flash:0731-cloud");
                assert_eq!(thinking.as_deref(), Some("high"));
                assert_eq!(fast_mode, Some(true));
                assert_eq!(context_window, Some(1_000_000));
                assert_eq!(max_output_tokens, Some(65_536));
            }
            other => panic!("unexpected variant: {other:?}"),
        }
        // Confirm fastMode + runtime overrides survive the round-trip.
        assert!(reser.contains("\"fastMode\":true"), "fastMode lost on reserialize: {reser}");
        assert!(reser.contains("\"contextWindow\":1000000"));
        assert!(reser.contains("\"maxOutputTokens\":65536"));
    }
