...
//   * all roles — Copy
//   * user      — Edit & resend
//   * assistant — Retry / regenerate, and Fork from here (when onFork is wired)
...
// Fork glyph — matches the Copy/Edit/Retry stroke style (12px, viewBox 24,
// currentColor). Mirrors the sessions ForkIcon path so the two surfaces share
// one visual language.
function ForkGlyph() {
  return (
    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
export function MessageActions({
  ...
  canRetry?: boolean;
  canEdit?: boolean;
  /** Fork a new session from this message (assistant only). */
  onFork?: () => void;
  /** Whether fork is available (non-streaming assistant message). */
  canFork?: boolean;
}) {
  ...
  return (
    ...
    >
      ...
      {role === "assistant" && canFork && onFork ? (
        <ActionButton label="Fork from here" title="Fork from here" onClick={onFork}>
          <ForkGlyph />
        </ActionButton>
      ) : null}
    </div>
