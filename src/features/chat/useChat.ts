...
export function useChat() {
  ...
  useIpcEvent((event) => {
    ...
    // Preserve compaction info from events if present (deep-shallow merge: if
    // the event carries compaction, merge it with the previous compaction).
    const eCompaction = e.compaction as Record<string, unknown> | undefined;
    const prevCompaction = (pendingStats.current ?? {}).compaction;
    const compaction = eCompaction
      ? { ...(prevCompaction ?? {}), ...(eCompaction as object) }
      : prevCompaction;
  ...
  const compact = useCallback(
    async (prompt?: string) => {
      await client.compact(prompt);
      // Refresh stats so the bar updates after compaction resolves.
      refreshContextStats();
    },
    [client, refreshContextStats],
  );
