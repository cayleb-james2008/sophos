...
describe("useModels", () => {
  ...
  it("fastMode defaults to false, toggles via setFastMode, and is included in setModel when on", async () => {
    const { result } = renderHook(() => useModels());
    await waitFor(() => expect(result.current.loading).toBe(false));

    // Default state: fastMode is false.
    expect(result.current.fastMode).toBe(false);

    // Toggle fastMode on.
    await act(async () => {
      await result.current.setFastMode(true);
    });
    expect(result.current.fastMode).toBe(true);

    // setSettings should have been called with the persisted fastMode flags.
    expect(mockState.client.setSettings).toHaveBeenCalledWith(
      expect.objectContaining({ defaultFastMode: true, fastMode: true }),
    );

    // setModel should now include fastMode=true as the trailing arg.
    (mockState.client.setModel as ReturnType<typeof vi.fn>).mockClear();
    await act(async () => {
      await result.current.setModel("ollama-cloud", "deepseek-v4-flash:0731-cloud", "medium");
    });
    expect(mockState.client.setModel).toHaveBeenCalledWith("ollama-cloud", "deepseek-v4-flash:0731-cloud", "medium", undefined, true);
  });
